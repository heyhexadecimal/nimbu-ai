...
// Get a valid Gmail client with automatic token refresh
  ...
  if (session.expiresAt && Date.now() > session.expiresAt - 60000) {
    if (session.refreshToken) {
      try {
        ...
        // For now, we'll use the refreshed token directly
...
// Initialize Gmail API client (legacy function - use getValidGmailClient for new code)
...
// Read recent emails with automatic token refresh
...
// Read recent emails (legacy function - use readEmailsWithRefresh for new code)
...
export async function sendEmail(
  ...
  try {
    ...
    const result = await gmail.users.messages.send({
      ...
      requestBody: {
        raw: encodedMessage,
        threadId: replyToMessageId ? undefined : undefined // Let Gmail handle threading
      }
    })
...
// Helper function to parse Gmail message format
